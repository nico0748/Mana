...
const MapPage: React.FC = () => {
  ...
  const venueMaps = useLiveQuery(() => db.venueMaps.toArray(), []);
  ...
  const saveMapDataUrl = async (hall: string, imageDataUrl: string) => {
    ...
    const now = Date.now();
    if (existing) {
      await db.venueMaps.update(existing.id, { imageDataUrl, generatedSvg: undefined, updatedAt: now });
    } else {
      await db.venueMaps.add({
  ...
  const handleGenerateSvg = async () => {
    ...
    try {
      ...
      await db.venueMaps.update(currentMap.id, { generatedSvg: svgString, updatedAt: Date.now() });
      setShowSvg(true);
    } catch (err) {
  ...
  const handleDeleteMap = async () => {
    ...
    await db.venueMaps.delete(currentMap.id);
    setEditMode(false);
    setSelectedCircleId(null);
  };
  ...
  const handleMapClick = useCallback(async (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    await db.circles.update(selectedCircleId, { mapX: x, mapY: y, updatedAt: Date.now() });
    setSelectedCircleId(null);
  }, [editMode, selectedCircleId, mapMode]);

  const handleRemovePin = async (circleId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    await db.circles.update(circleId, { mapX: undefined, mapY: undefined, updatedAt: Date.now() });
  };
