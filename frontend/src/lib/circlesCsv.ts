...
/**
 * Parse an Excel workbook buffer into an array of normalized circle rows.
 *
 * Parses the first worksheet, maps header columns to CircleRow fields, trims cell values,
 * enforces presence of required columns, and normalizes `status` to one of the allowed values
 * (defaults to `pending` when missing or invalid).
 *
 * @param buffer - ArrayBuffer containing an XLSX workbook
 * @returns An array of parsed `CircleRow` objects with fields: `name`, `author`, `hall`, `block`, `number`, and `status`
 * @throws Error with message "データが空です" if the sheet contains fewer than two non-note rows
 * @throws Error with message "「サークル名」列が見つかりません" if the name column is missing
 * @throws Error with message "「ホール」列が見つかりません" if the hall column is missing
 * @throws Error with message "インポートできる行がありません" if no valid rows are found after parsing
 */
...
/**
 * Generate and download an Excel template for importing circles.
 *
 * Creates a workbook containing the template headers, an example row, an empty spacer row, and a status note,
 * sets fixed column widths, adds the sheet named "買い物リスト", and saves the file as "買い物リスト_テンプレート.xlsx".
 */
