# mana-library デプロイ要件定義

## アーキテクチャ概要

mana-library は **完全クライアントサイドの静的 SPA** である。
...
プロジェクト名が `mana-library` であり、将来 Firestore/Authentication を追加する可能性があるため最も親和性が高い。
...
> **前提条件（重要）**
>
> 現在の mana-library は **個人の蔵書管理ツール**であり、データはすべてブラウザのローカルストレージ（IndexedDB）に保存される。
...
<title>Mana Library — 蔵書管理アプリ</title>
...
<!-- OGP（SNS シェア時のカード表示） -->
<meta property="og:title" content="Mana Library — 蔵書管理アプリ" />
...
<meta name="twitter:title" content="Mana Library — 蔵書管理アプリ" />
